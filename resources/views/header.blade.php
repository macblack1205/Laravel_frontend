    ...
    <header class="header">
        ...
        <div id="upper-header" class="flex flex-row justify-between">
            <div class="hidden md:flex dual-button">
                <button onclick="location.href='{{ route('dashboard') }}'" class="dual-black">Main</button>
                <button onclick="location.href='{{ route('home') }}'" class="dual-white">Home</button>
            </div>
            
        </div>
        @else
        <div class="hidden md:flex dual-button">
            <button onclick="location.href='{{ route('login') }}'" class="dual-black">Sign In</button>
            <button  onclick="location.href='{{ route('register') }}'" class="dual-white">Sign Up</button>
        </div>
        @endif
        ...
        <div class="hidden md:flex flex-row p-0 m-0 space-x-1 items-center">
            @if($user)
                <p class="text-base font-absans font-regular">Welcome, <span class="text-xl cursor-auto font-bold font-absans hover:underline hover:underline-offset-4">{{ $user['first'] }}  {{ $user['last'] }}!</span></p>
                ...
                <form action="{{ route('logoutApi') }}" method="POST" class="inline-block border-0 p-0 m-0">
                    @csrf
